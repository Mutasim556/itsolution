$(document).on('submit', '#add_wing_form', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (rdata) {
            $('button[type=submit]', '#add_wing_form').html(submit_btn_before);
            $('button[type=submit]', '#add_wing_form').removeClass('disabled');
            swal({
                icon: "success",
                title: rdata.title,
                text: rdata.text,
                ...
                let wingM_image = data.logo ? '<img style="height: 50px;width:50px;" src="' + base_url + '/' + data.logo + '">' : no_file;


                $('#basic-1 tbody').append(`<tr id="trid-${data.id}" data-id="${data.id}"><td>${wingM_image}</td><td>${data.name}</td><td>${data.link}</td>
                ...
                $('#add_wing_form .err-mgs').each(function (id, val) {
                    $(this).prev('input').removeClass('border-danger is-invalid')
                    $(this).prev('textarea').removeClass('border-danger is-invalid')
                    $(this).empty();
                })
                $('#add_wing_form').trigger('reset');
                $('button[type=button]', '#add_wing_form').click();
            })
        },
        error: function (err) {
            $('button[type=submit]', '#add_wing_form').html(submit_btn_before);
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#add_wing_form #' + exp).addClass('border-danger is-invalid')
                $('#add_wing_form #' + exp2).addClass('border-danger is-invalid')
                $('#add_wing_form #' + exp).next('span').find('.select2-selection--single').attr('id', 'invalid-selec2')
                $('#add_wing_form #' + exp).next('.err-mgs').empty().append(val);

                $('#add_wing_form #' + exp + "_err").empty().append(val);
            })
        },
    })
});
...
$(document).on('change', '#status_change', function () {
    ...
    $.ajax({
        type: "get",
        url: 'wings/update/status/' + update_id + "/" + status,
...
$(document).on('click', '#edit_button', function () {
    $('#edit_wing_form').trigger('reset');
    $('#edit_wing_form .err-mgs').each(function (id, val) {
        $(this).prev('input').removeClass('border-danger is-invalid')
        $(this).prev('textarea').removeClass('border-danger is-invalid')
        $(this).empty();
    })
    ...
    $.ajax({
        ...
        success: function (data) {
            $('#edit_wing_form #wing_id').val(data.id);
            $('#edit_wing_form #name').val(data.name);
            $('#edit_wing_form #link').val(data.link);

            $('#edit_wing_form #eprev_logo').prop('src', base_url + "/" + data.logo);
            $.each(data.translations, function (key, val) {
                if (val.locale == 'en') {
                } else {
                    if (val.key == 'name') {
                        $('#edit_wing_form #name_' + val.locale).val(val.value);
                    }
                }
            })
        },
        error: function (err) {
            if (err.status === 403) {
                ...
                swal({
                    ...
                    $('button[type=button]', '#edit_wing_form').click();
                });
...
$('#edit_wing_form').submit(function (e) {
    e.preventDefault();
    $('button[type=submit]', this).html(submit_btn_after + '....');
    $('button[type=submit]', this).addClass('disabled');
    var trid = '#trid-' + $('#wing_id', this).val();
    ...
    $.ajax({
        type: "post",
        url: 'wings/' + $('#wing_id', '#edit_wing_form').val(),
        data: formData,
        dataType: 'JSON',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        processData: false,
        success: function (data) {
            $('button[type=submit]', '#edit_wing_form').html(submit_btn_before);
            $('button[type=submit]', '#edit_wing_form').removeClass('disabled');

            $('td:nth-child(1)', trid).html(data.wing.logo ? `<img style="height: 50px;" src="${base_url + '/' + data.wing.logo}">` : no_file);
            $('td:nth-child(2)', trid).html(data.wing.name);
        ...
        error: function (err) {
            ...
            $.each(err.responseJSON.errors, function (idx, val) {
                ...
                $('#edit_wing_form #' + idx).addClass('border-danger is-invalid')
                $('#edit_wing_form #' + idx).next('.err-mgs').empty().append(val);
            })
        }
    });
});
